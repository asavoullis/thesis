...
if nargin == 6 %simple version for Code rate=1 and preamble length=8
    CR = 1;
    n_preamble = 8 ;
    SyncKey = 5 ;
elseif nargin == 7
...
signal_mod = 10.^(Pt./20).*signal.*exp(-j.*2.*pi.*df/Fs.*(0:length(signal)-1))' ; % frquency shift and convert to volt
end
...
M       = 2^SF ;  % number of possible chirp values
