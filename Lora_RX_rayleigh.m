function [message] = Lora_RX_rayleigh(signal,Bandwidth,SF,Coherece,Fs,df,varargin)
...
if Fs == Bandwidth
    signal_demod        = rayleigh(100,10000,150,3,0.00000001);
    signal_demod        = signal_demod(1:4512);
else
    signal_freq_demod   = signal.*exp(j.*2.*pi.*df./Fs.*(0:length(signal)-1))' ;
    signal_filter       = lowpass(signal_freq_demod,Bandwidth,Fs) ;
    %signal_demod1       = awgn(resample(signal_filter,Bandwidth,Fs),SNR,'measured') ;
    %disp(signal_demod1);
    signal_demod        = rayleigh(1000,10000,1500,3,0.01);
    %signal_demod2        = signal_demod(1:4512);
    %disp(signal_demod);
